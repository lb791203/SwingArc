...
1. Mac 当时处于锁屏，无法通过 Simulator UI 点按完成 A/B 双人标注、
   四帧分歧裁决、关键点拖动、强制退出恢复及页面截图。
2. 无线 iPhone 16 Pro
...
4. 标注者 A 提交 P1–P8，标注者 B 提交前不可见 A 的答案；
5. 制造四帧分歧并完成裁决；
6. 放大并移动 grip、shaftStart、shaftEnd、clubhead、ball，设置可见性和
   人工复核；
7. 活跃修订期间强制退出，重启后核对当前帧、队列、复核状态和人工锁定；
8. 在 iPhone 和常规宽度 iPad 布局各走一遍；
9. 导出 JSON 并用 Mac 校验器确认 `VALID`；
