...
| 正面视角 | 代码覆盖，待单独截屏 | 与 DTL 共用练习流程，以视角标签和建议模型切换；尚未单独进行模拟器截图验收。 |
...
- 本轮截图：`/tmp/swingarc-pro-tour-home.png`、`/tmp/swingarc-pro-tour-alignment-2.png`、`/tmp/swingarc-pro-tour-ready-2.png`
- 仅用于开发构建的预览参数：`-swingarc-preview-dtl`、`-swingarc-preview-ready`；实现均在 `#if DEBUG` 中，发布构建不受影响。
