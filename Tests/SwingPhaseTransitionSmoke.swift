...
struct SwingPhaseTransitionSmoke {
    static func main() {
        let samples = [
            ...
            make(0.48, 0.38), make(0.56, 0.62), // sustained downswing; P7 is fast.
            make(0.64, 0.67), // lower hand position but slower, not impact.
            make(0.72, 0.50), make(0.80, 0.44), make(0.88, 0.44),
            make(0.96, 0.65), make(1.04, 0.65) // later clip tail, not a shaft-defined P8.
        ]
        ...
        precondition(detections.first(where: { $0.stage == .shaftParallelDownswing })?.status == .unresolved)
        precondition(detections.first(where: { $0.stage == .impact })?.time == 0.56)
        precondition(detections.first(where: { $0.stage == .followThrough })?.status == .unresolved)
        precondition(!detections.contains { $0.stage == .finish })
    }
