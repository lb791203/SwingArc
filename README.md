...
开发构建仅提供模拟器画面检查参数：`-swingarc-preview-dtl`、`-swingarc-preview-ready`、`-swingarc-preview-face-on`、`-swingarc-preview-face-on-ready`、`-swingarc-preview-library`，以及配合本地测试影片的 `-swingarc-preview-import` / `-swingarc-preview-analysis`。它们被 `#if DEBUG` 隔离，不会进入发布构建或改变真机正常流程。
