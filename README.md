...
开发构建仅提供模拟器画面检查参数：`-swingarc-preview-dtl`、`-swingarc-preview-ready`、`-swingarc-preview-face-on` 与 `-swingarc-preview-face-on-ready`。它们被 `#if DEBUG` 隔离，不会进入发布构建或改变真机正常流程。
